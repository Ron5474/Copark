...
test('OAuth user with id throws error', async () => {
  ...
  await expect(new AuthService().driverLogin(driver)).rejects.toThrow('Unauthorized')
})
...
test('getOauthUser() user with id throws error', async () => {
  ...
  await expect(new AuthService().getOauthUser(driver)).rejects.toThrow('Unauthorized')
})
