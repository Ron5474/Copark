...
test('Server starts and calls bootstrap', async () => {
  ...
  expect(listenSpy).toHaveBeenCalledWith(4002, expect.any(Function));
  expect(bootstrap).toHaveBeenCalled();
  expect(console.log).toHaveBeenCalledWith('Running a GraphQL Playground at http://localhost:4002/playground');
