...
import { vi, it, beforeAll, afterAll, afterEach, expect } from 'vitest'
import { setupServer } from 'msw/node'
import { ticketHandlers, pictureHandlers } from './mockService'
import { issueTicket } from '@/app/dashboard/violation/actions'
import { scanPlate } from '@/app/dashboard/camera/actions'
...
const server = setupServer(
  ticketHandlers.success,
  pictureHandlers.success
)
...
it('returns plate and state from recognizePlate', async () => {
  const result = await scanPlate('data:image/png;base64,test-image')
...
it('throws if recognizePlate returns a GraphQL error', async () => {
  server.use(pictureHandlers.graphqlError)

  await expect(() =>
    scanPlate('data:image/png;base64:bad')
  ).rejects.toThrow('Image is too blurry')
})


it('throws if HTTP response is not ok (500)', async () => {
  server.use(pictureHandlers.failure)

  await expect(() =>
    scanPlate('data:image/png;base64:crash')
  ).rejects.toThrow('GraphQL service crashed')
})
